...
Folia-важно: renderer не вызывает Bukkit `ScoreboardManager#getNewScoreboard()`, `Scoreboard#registerNewObjective()` и другие scoreboard-mutation API, потому что на Folia эти пути могут бросать `UnsupportedOperationException`. Scoreboard доставляется через PacketEvents clientbound packets (`ScoreboardObjective`, `DisplayScoreboard`, `Teams`, `UpdateScore`, `ResetScore`), поэтому HudOrchestrator не мутирует Bukkit scoreboard state и не зависит от неполной Folia-реализации scoreboard API. Runtime-зависимость `packetevents` обязательна.
